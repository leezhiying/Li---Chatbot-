...
from keras.layers import Input, LSTM, Dense, Embedding, TimeDistributed,Bidirectional,Concatenate,GRU
...
input_texts_new = cornell_input + squad_input
target_texts_new = cornell_output + squad_output

input_counter = cornell_input_counter + squad_input_counter 
target_counter  = cornell_output_counter + squad_output_counter 
...
def train(epochs=NUM_EPOCHS, BATCH_SIZE = BATCH_SIZE, optimizer = optimizer):
    ...
    encoder_lstm = LSTM(units=HIDDEN_UNITS, return_state=True, name='encoder_lstm',return_sequences=True)

    #encoder_lstm = Bidirectional(LSTM(units=HIDDEN_UNITS, name='encoder_lstm',return_sequences=True),input_shape = (encoder_max_seq_length,1))

    encoder_outputs, encoder_state_h, encoder_state_c = (encoder_lstm(encoder_embedding(encoder_inputs)))
    #encoder_outputs, encoder_state_h, encoder_state_c = Bidirectional(encoder_lstm(encoder_embedding(encoder_inputs)),input_shape=(encoder_max_seq_length, 1))
    ...
    encoder_inputs = Input(shape=(None,), name = 'encode_inputs',dtype='int32')
 
    encoder_embedding = Embedding(input_dim=num_encoder_tokens, output_dim=HIDDEN_UNITS,
                                  input_length=encoder_max_seq_length, name='encoder_embedding')(encoder_inputs)
 

    encoder_lstm, forward_h, forward_c, backward_h, backward_c =  Bidirectional(LSTM
         (HIDDEN_UNITS,
          dropout=0.2,
    ...
    output = Dense(units=num_decoder_tokens, activation='softmax', name='decoder_dense')(decoder_outputs)

    model = Model([encoder_inputs,decoder_inputs], outputs=output)
    model.compile(loss='categorical_crossentropy', optimizer='rmsprop')
    # summarize layers
    print(model.summary())

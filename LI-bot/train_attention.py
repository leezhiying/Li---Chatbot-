...
X_cornell = create_dataset("data/cornell.in")
Y_cornell = create_dataset("data/cornell.ou")

X_squad = create_dataset("data/squad.in")
Y_squad = create_dataset("data/squad.in")

X_wiki = create_dataset("data/wiki.in")
Y_wiki = create_dataset("data/wiki.in")


X = X_cornell + X_squad + X_wiki
Y = Y_cornell + Y_squad + Y_wiki
...
for (item_x,item_y) in zip(X,Y):
    if len(item_x) < MAX_INPUT_SEQ_LENGTH and len(item_y)<MAX_TARGET_SEQ_LENGTH:
        X_new.append(item_x)
        Y_new.append(item_y)
...
input_tensor, inp_lang = tokenize(tuple(X_new))
target_tensor, targ_lang = tokenize(tuple(Y_new))
...
chat('What is your name?', encoder, decoder, inp_lang, targ_lang, max_length_inp, max_length_targ)
chat('Are you a follower or a leader?', encoder, decoder, inp_lang, targ_lang, max_length_inp, max_length_targ)
